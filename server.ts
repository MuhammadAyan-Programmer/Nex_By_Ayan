...
/*
 * IMPORTANT:
 * Use the PORT provided by the hosting platform.
 * Locally it will fall back to 3000.
 */
  ...
  return (
    ...
    Boolean(
      process.env.ADMIN_EMAIL &&
        norm === process.env.ADMIN_EMAIL.trim().toLowerCase()
    )
  );
};
...
const DEFAULT_PROJECTS = [
  {
    ...
    communityLink:
      'https://community.nexora.work/c/arabic-english-translation',
    announcement:
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (
    isValidAdminEmail(normalizedEmail) &&
    isValidAdminPassword(inputPassword)
  ) {
